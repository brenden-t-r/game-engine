...
public:
    ...
    public:
        explicit TextureD3D(const char* path, ID3D11ShaderResourceView* textureView): Texture(path), textureView(textureView) {}
        ~TextureD3D() override {
            textureView->Release();
        }
    ...
    Texture* CreateTexture(const char* path) override {
        ...
        delete[] wchar;
        auto texture = new TextureD3D(path, textureView);
        return texture;
    }
    //endregion
    //region Sprite
